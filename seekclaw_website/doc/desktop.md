...
- **全局任务**不绑定项目，适合通用问答；它不会使用本地文件、终端或 Git 工具。

新建任务时不必立即创建 Runtime Session。只有第一次发送消息后，Desktop 才会创建并持久化会话。侧栏中的“全局任务”用于展开全局任务列表，而不是强制切换到某个会话。

任务标题会根据首条提示词生成。你可以归档、恢复或删除任务，也可以批量处理一个项目或全局范围内的任务。已归档任务为只读状态。删除项目会永久删除该项目下的全部会话（包括已归档会话），但不会删除本地项目文件。

## 发起一次对话

1. 点击“新建任务”，选择一个项目，或使用不绑定目录的全局任务。
