...
所有内置 Prompt 必须独立存储于外部文件，禁止将任何长 Prompt、System Prompt、Developer Prompt、Tool Prompt、Skill Prompt、模板或工作流提示词直接硬编码在 C# 源代码中。
...
Skill 启用时自动加入 Prompt。

---

# MCP Prompt

MCP Server 提供 Prompt 时：

自动加入 Prompt Registry。
...
Prompt 必须完全数据化（Data Driven）。

新增或修改 Prompt 时，应尽可能无需修改 C# 源代码，仅通过新增、替换或编辑 Prompt 文件即可完成。

Prompt 管理系统应支持热加载、缓存、变量替换、组合、版本管理及插件扩展，为未来社区 Prompt、Skill Prompt 和自定义 Prompt 提供统一的基础设施。
